...
client.once('ready', () => {
    ...
    setInterval(() => {
        ...
        let presence = { 
            ...
            metadata : {
                title : (out.find(arg => arg.startsWith('tag title')) || []).split(' ').slice(2),
                artist : (out.find(arg => arg.startsWith('tag artist')) || []).split(' ').slice(2),
                album: (out.find(arg => arg.startsWith('tag album')) || []).split(' ').slice(2)
            }
        };
